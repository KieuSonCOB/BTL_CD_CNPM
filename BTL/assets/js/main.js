	...
	$('.form_search').submit(function(event){
		...
		$('.product').each(function(){
			...
			if(productName.includes(searchTerm)){
				$("#searched-products").addClass("show");
...
function go_to_page(page_num) {
	...
	$('#pop1 .product').css('display', 'none').slice(start_from, end_on).css('display', 'block');
...
function check_navigation_buttons() {
	...
	var number_of_pages = Math.ceil($('#pop1 .product').length / parseInt($('#show_per_page').val()));
...
jQuery(document).ready(function () {
	$('.form_search').on('submit', function () {
		...
		var navigation_html = '<a class="previous_link" href="javascript:previous2();">&laquo;</a>';
		var current_link = 0;
		while (number_of_pages > current_link) {
			navigation_html += '<a class="page_link2" href="javascript:go_to_page2(' + current_link + ')" longdesc="' + current_link + '">' + (current_link + 1) + '</a>';
			current_link++;
		}
		navigation_html += '<a class="next_link" href="javascript:next2();">&raquo;</a>';
		...
		$('.searched-products .product').slice(0, show_per_page).css('display', 'block');
...
function previous2() {
	...
	if ($('#page_navigation2 .active_page').prev('#page_navigation2 .page_link2').length == true) {
...
function next2() {
	new_page = parseInt($('#current_page2').val()) + 1;
	if ($('#page_navigation2 .active_page').next('#page_navigation2 .page_link2').length == true) {
		go_to_page2(new_page);
	}
	check_navigation_buttons2();
}
function go_to_page2(page_number) {
	...
	$('.searched-products .product').css('display', 'none').slice(start_from, end_at).css('display', 'block');
	$('#page_navigation2 .page_link2[longdesc=' + page_number + ']').addClass('active_page').siblings('.active_page').removeClass('active_page');
	$('#current_page2').val(page_number);
	check_navigation_buttons2();
}
function check_navigation_buttons2() {
	var current_page = parseInt($('#current_page2').val());
	var number_of_pages = Math.ceil($('.searched-products .product').length / parseInt($('#show_per_page2').val()));
...
jQuery(document).ready(function () {
	$('.loai-category').on('click', function () {
		...
		var navigation_html = '<a class="previous_link" href="javascript:previous3();">&laquo;</a>';
		var current_link = 0;
		while (number_of_pages > current_link) {
			navigation_html += '<a class="page_link3" href="javascript:go_to_page3(' + current_link + ')" longdesc="' + current_link + '">' + (current_link + 1) + '</a>';
			current_link++;
		}
		navigation_html += '<a class="next_link" href="javascript:next3();">&raquo;</a>';
		...
		$('.fill-products .product').slice(0, show_per_page).css('display', 'block');
...
function previous3() {
	...
	if ($('#page_navigation3 .active_page').prev('#page_navigation3 .page_link3').length == true) {
...
function next3() {
	new_page = parseInt($('#current_page3').val()) + 1;
	if ($('#page_navigation3 .active_page').next('#page_navigation3 .page_link3').length == true) {
		go_to_page3(new_page);
	}
	check_navigation_buttons3();
}
function go_to_page3(page_number) {
	...
	start_from = page_number * show_per_page;
	end_at = start_from + show_per_page;
	$('.fill-products .product').css('display', 'none').slice(start_from, end_at).css('display', 'block');
	$('#page_navigation3 .page_link3[longdesc=' + page_number + ']').addClass('active_page').siblings('.active_page').removeClass('active_page');
	$('#current_page3').val(page_number);
	check_navigation_buttons3();
}
function check_navigation_buttons3() {
	var current_page = parseInt($('#current_page3').val());
	var number_of_pages = Math.ceil($('.fill-products .product').length / parseInt($('#show_per_page3').val()));
